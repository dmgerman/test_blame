...
macro_line|#define MAX_AUTO_MIC_PINS&t;3
r_struct
id|alc_automic_entry
(brace
"&t;"
id|hda_nid_t
id|pin
suffix:semicolon
"&t;&t;"
multiline_comment|/* pin */
"&t;"
r_int
id|idx
suffix:semicolon
"&t;&t;"
multiline_comment|/* imux index, -1 = invalid */
"&t;"
r_int
r_int
id|attr
suffix:semicolon
"&t;"
multiline_comment|/* pin attribute (INPUT_PIN_ATTR_*) */
)brace
suffix:semicolon
...
comma
"&t;"
id|NID_PATH_BOOST_CTL
comma
"&t;"
id|NID_PATH_NUM_CTLS
)brace
suffix:semicolon
multiline_comment|/* Widget connection path&n; *&n; * For output, stored in the order of DAC -&gt; ... -&gt; pin,&n; * for input, pin -&gt; ... -&gt; ADC.&n; *&n; * idx[i] contains the source index number to select on of the widget path[i];&n; * e.g. idx[1] is the index of the DAC (path[0]) selected by path[1] widget&n; * multi[] indicates whether it&squot;s a selector widget with multi-connectors&n; * (i.e. the connection selection is mandatory)&n; * vol_ctl and mute_ctl contains the NIDs for the assigned mixers&n; */
r_struct
id|nid_path
(brace
"&t;"
...
suffix:semicolon
multiline_comment|/* NID_PATH_XXX_CTL */
"&t;"
r_bool
id|active
suffix:semicolon
...
suffix:semicolon
"&t;"
multiline_comment|/* min. channel count (for speakers) */
"&t;"
r_int
id|ext_channel_count
suffix:semicolon
"&t;&t;"
multiline_comment|/* current channel count for multi-io */
"&t;"
...
suffix:semicolon
"&t;"
multiline_comment|/* auto-mic stuff */
"&t;"
r_int
id|am_num_entries
suffix:semicolon
...
r_int
r_int
id|keep_vref_in_automute
suffix:colon
l_int|1
suffix:semicolon
multiline_comment|/* Don&squot;t clear VREF in automute */
"&t;"
r_int
r_int
id|line_in_auto_switch
suffix:colon
l_int|1
suffix:semicolon
multiline_comment|/* allow line-in auto switch */
"&t;"
...
l_int|1
suffix:semicolon
multiline_comment|/* allow multiple capture xxx volumes */
"&t;"
r_int
r_int
id|inv_dmic_split
...
l_int|1
suffix:semicolon
multiline_comment|/* inverted dmic w/a for conexant */
"&t;"
r_int
r_int
id|parse_flags
...
suffix:semicolon
multiline_comment|/* for shared I/O, change the pin-control accordingly */
r_void
id|update_shared_mic_hp
c_func
(paren
r_struct
id|hda_codec
...
suffix:semicolon
"&t;"
multiline_comment|/* NOTE: this assumes that there are only two inputs, the&n;&t; * first is the real internal mic and the second is HP/mic jack.&n;&t; */
"&t;"
id|val
op_assign
id|snd_hda_get_default_vref
c_func
(paren
id|codec
comma
...
suffix:semicolon
"&t;"
multiline_comment|/* This pin does not have vref caps - let&squot;s enable vref on pin 0x18&n;&t;   instead, as suggested by Realtek */
"&t;"
r_if
c_cond
(paren
...
suffix:semicolon
"&t;&t;"
multiline_comment|/* don&squot;t reset VREF value in case it&squot;s controlling&n;&t;&t; * the amp (see alc861_fixup_asus_amp_vref_0f())&n;&t;&t; */
"&t;&t;"
r_if
c_cond
(paren
id|spec-&gt;keep_vref_in_automute
)paren
(brace
"&t;&t;&t;"
id|val
...
multiline_comment|/* don&squot;t change HP-pin when shared with mic */
"&t;&t;"
id|do_automute
c_func
(paren
...
"&t;"
r_return
id|snd_hda_enum_bool_helper_info
c_func
(paren
id|kcontrol
comma
...
"&t;"
multiline_comment|/* we don&squot;t need the jack detection for the first pin */
"&t;"
r_for
c_loop
(paren
...
l_int|0
suffix:semicolon
multiline_comment|/* only mic is allowed */
"&t;&t;&t;"
r_if
c_cond
(paren
...
suffix:semicolon
"&t;"
multiline_comment|/* sort the am_entry in the order of attr so that the pin with a&n;&t; * higher attr will be selected when the jack is plugged.&n;&t; */
"&t;"
id|sort
c_func
(paren
...
macro_line|#define alc_cap_vol_info&t;snd_hda_mixer_amp_volume_info
macro_line|#define alc_cap_vol_get&t;&t;snd_hda_mixer_amp_volume_get
macro_line|#define alc_cap_vol_tlv&t;&t;snd_hda_mixer_amp_tlv
r_typedef
r_int
(paren
op_star
...
macro_line|#define alc_cap_sw_get&t;&t;snd_hda_mixer_amp_switch_get
r_int
id|alc_cap_sw_put
c_func
(paren
...
c_cond
(paren
id|spec-&gt;vmaster_mute.hook
)paren
"&t;&t;&t;"
id|snd_hda_add_vmaster_hook
c_func
(paren
id|codec
comma
...
suffix:semicolon
"&t;"
multiline_comment|/* if there is really a single DAC used in the whole output paths,&n;&t; * use it master (or &quot;PCM&quot; if a vmaster hook is present)&n;&t; */
"&t;"
r_if
c_cond
(paren
...
l_int|0
suffix:semicolon
multiline_comment|/* no need for analog loopback */
"&t;"
id|path
op_assign
id|add_new_nid_path
...
multiline_comment|/* check whether the given two widgets can be connected */
r_bool
id|is_reachable_path
c_func
(paren
r_struct
id|hda_codec
...
multiline_comment|/* Parse the codec tree and retrieve ADCs */
r_int
id|alc_auto_fill_adc_nids
c_func
(paren
r_struct
id|hda_codec
...
multiline_comment|/* filter out invalid adc_nids that don&squot;t give all active input pins;&n; * if needed, check whether dynamic ADC-switching is available&n; */
r_int
id|check_dyn_adc_switch
c_func
(paren
r_struct
id|hda_codec
...
c_func
(paren
l_string|&quot;realtek: reducing to a single ADC&bslash;n&quot;
)paren
suffix:semicolon
"&t;&t;"
id|spec-&gt;num_adc_nids
...
"&t;"
multiline_comment|/* single index for individual volumes ctls */
"&t;"
r_if
c_cond
(paren
...
multiline_comment|/* templates for capture controls */
r_const
r_struct
id|snd_kcontrol_new
id|cap_src_temp
...
op_or
"&t;&t;"
id|SNDRV_CTL_ELEM_ACCESS_TLV_READ
op_or
"&t;&t;"
id|SNDRV_CTL_ELEM_ACCESS_TLV_CALLBACK
)paren
comma
"&t;"
dot
...
suffix:semicolon
"&t;"
multiline_comment|/* Make independent right kcontrol */
"&t;"
r_if
c_cond
(paren
...
multiline_comment|/* create single (and simple) capture volume and switch controls */
r_int
id|create_single_cap_vol_ctl
c_func
(paren
r_struct
id|hda_codec
...
multiline_comment|/* create bound capture volume and switch controls */
r_int
id|create_bind_cap_vol_ctl
c_func
(paren
r_struct
id|hda_codec
...
multiline_comment|/* return the vol ctl when used first in the imux list */
r_int
r_int
id|get_first_cap_ctl
c_func
(paren
r_struct
id|hda_codec
...
multiline_comment|/* create individual capture volume and switch controls per input */
r_int
id|create_multi_cap_vol_ctl
c_func
(paren
r_struct
id|hda_codec
...
id|KERN_ERR
"&t;&t;&t;&t;&t;"
l_string|&quot;invalid input path 0x%x -&gt; 0x%x&bslash;n&quot;
comma
"&t;&t;&t;&t;&t;"
id|pin
comma
...
multiline_comment|/* create a shared input with the headphone out */
r_int
id|alc_auto_create_shared_input
c_func
(paren
r_struct
id|hda_codec
...
suffix:semicolon
"&t;"
multiline_comment|/* only one internal input pin? */
"&t;"
r_if
c_cond
(paren
...
suffix:semicolon
multiline_comment|/* OK, we have a single HP-out */
"&t;"
r_else
r_if
c_cond
...
suffix:semicolon
multiline_comment|/* OK, we have a single line-out */
"&t;"
r_else
"&t;&t;"
r_return
l_int|0
suffix:semicolon
multiline_comment|/* both not available */
"&t;"
r_if
c_cond
(paren
...
c_func
(paren
l_string|&quot;realtek: Enable shared I/O jack on NID 0x%x&bslash;n&quot;
comma
id|nid
)paren
suffix:semicolon
...
suffix:semicolon
"&t;&t;"
multiline_comment|/* mute loopback inputs */
"&t;&t;"
r_if
c_cond
(paren
...
multiline_comment|/* called recursively */
r_bool
id|__parse_nid_path
c_func
(paren
...
l_int|2
suffix:semicolon
multiline_comment|/* mark aa-mix is included */
"&t;"
)brace
"&t;"
id|nums
...
(brace
"&t;&t;&t;"
multiline_comment|/* special case: when from_nid is 0,&n;&t;&t;&t; * try to find an empty DAC&n;&t;&t;&t; */
"&t;&t;&t;"
r_if
c_cond
(paren
...
"&t;&t;"
multiline_comment|/* aa-mix is requested but not included? */
"&t;&t;"
r_if
c_cond
(paren
...
multiline_comment|/* parse the widget path from the given nid to the target nid;&n; * when @from_nid is 0, try to find an empty DAC;&n; * when @with_aa_mix is 0, paths with spec-&gt;mixer_nid are excluded.&n; * when @with_aa_mix is 1, paths without spec-&gt;mixer_nid are excluded.&n; * when @with_aa_mix is 2, no special handling about spec-&gt;mixer_nid.&n; */
r_bool
id|parse_nid_path
c_func
(paren
...
c_func
(paren
l_string|&quot;path: depth=%d, %02x/%02x/%02x/%02x/%02x&bslash;n&quot;
comma
"&t;&t;&t;"
id|path-&gt;depth
comma
...
multiline_comment|/* badness definition */
r_enum
(brace
"&t;"
multiline_comment|/* No primary DAC is found for the main output */
"&t;"
id|BAD_NO_PRIMARY_DAC
op_assign
l_int|0x10000
comma
"&t;"
multiline_comment|/* No DAC is found for the extra output */
"&t;"
id|BAD_NO_DAC
op_assign
l_int|0x4000
comma
"&t;"
multiline_comment|/* No possible multi-ios */
"&t;"
id|BAD_MULTI_IO
op_assign
l_int|0x103
comma
"&t;"
multiline_comment|/* No individual DAC for extra output */
"&t;"
id|BAD_NO_EXTRA_DAC
op_assign
l_int|0x102
comma
"&t;"
multiline_comment|/* No individual DAC for extra surrounds */
"&t;"
id|BAD_NO_EXTRA_SURR_DAC
op_assign
l_int|0x101
comma
"&t;"
multiline_comment|/* Primary DAC shared with main surrounds */
"&t;"
id|BAD_SHARED_SURROUND
op_assign
l_int|0x100
comma
"&t;"
multiline_comment|/* Primary DAC shared with main CLFE */
"&t;"
id|BAD_SHARED_CLFE
op_assign
l_int|0x10
comma
"&t;"
multiline_comment|/* Primary DAC shared with extra surrounds */
"&t;"
id|BAD_SHARED_EXTRA_SURROUND
op_assign
l_int|0x10
comma
"&t;"
multiline_comment|/* Volume widget is shared */
"&t;"
id|BAD_SHARED_VOL
op_assign
l_int|0x10
...
multiline_comment|/* get the path between the given NIDs;&n; * passing 0 to either @pin or @dac behaves as a wildcard&n; */
r_struct
id|nid_path
op_star
id|get_nid_path
...
multiline_comment|/* look for widgets in the path between the given NIDs appropriate for&n; * volume and mute controls, and assign the values to ctls[].&n; *&n; * When no appropriate widget is found in the path, the badness value&n; * is incremented depending on the situation.  The function returns the&n; * total badness for both volume and mute controls.&n; */
r_int
id|assign_out_path_ctls
c_func
(paren
r_struct
id|hda_codec
...
suffix:semicolon
"&t;"
multiline_comment|/* no primary DAC */
"&t;"
r_int
id|no_dac
suffix:semicolon
"&t;&t;"
multiline_comment|/* no secondary DACs */
"&t;"
r_int
id|shared_primary
suffix:semicolon
"&t;"
multiline_comment|/* primary DAC is shared with main output */
"&t;"
r_int
id|shared_surr
suffix:semicolon
"&t;"
multiline_comment|/* secondary DAC shared with main or primary */
"&t;"
r_int
id|shared_clfe
suffix:semicolon
"&t;"
multiline_comment|/* third DAC shared with main or primary */
"&t;"
r_int
id|shared_surr_main
suffix:semicolon
"&t;"
multiline_comment|/* secondary DAC sahred with main/DAC0 */
)brace
suffix:semicolon
r_struct
id|badness_table
...
suffix:semicolon
multiline_comment|/* try to assign DACs to pins and return the resultant badness */
r_int
id|alc_auto_fill_dacs
c_func
(paren
r_struct
id|hda_codec
...
suffix:semicolon
"&t;&t;"
multiline_comment|/* we don&squot;t count badness at this stage yet */
"&t;"
)brace
"&t;"
r_if
...
macro_line|#define DEBUG_BADNESS
macro_line|#ifdef DEBUG_BADNESS
macro_line|#define debug_badness&t;snd_printdd
macro_line|#else
macro_line|#define debug_badness(...)
macro_line|#endif
r_void
id|debug_show_configs
c_func
(paren
r_struct
id|alc_spec
...
c_func
(paren
l_string|&quot;multi_outs = %x/%x/%x/%x : %x/%x/%x/%x&bslash;n&quot;
comma
"&t;&t;"
id|cfg-&gt;line_out_pins
(braket
...
c_func
(paren
l_string|&quot;multi_ios(%d) = %x/%x : %x/%x&bslash;n&quot;
comma
"&t;&t;&t;"
id|spec-&gt;multi_ios
comma
...
c_func
(paren
l_string|&quot;hp_outs = %x/%x/%x/%x : %x/%x/%x/%x&bslash;n&quot;
comma
"&t;&t;"
id|cfg-&gt;hp_pins
(braket
...
c_func
(paren
l_string|&quot;spk_outs = %x/%x/%x/%x : %x/%x/%x/%x&bslash;n&quot;
comma
"&t;&t;"
id|cfg-&gt;speaker_pins
(braket
...
multiline_comment|/* find all available DACs of the codec */
r_void
id|alc_fill_all_nids
c_func
(paren
r_struct
id|hda_codec
...
(paren
id|KERN_ERR
l_string|&quot;hda: Too many DACs!&bslash;n&quot;
)paren
suffix:semicolon
"&t;&t;&t;"
r_break
...
c_func
(paren
l_string|&quot;==&gt; lo_type=%d, wired=%d, mio=%d, badness=0x%x&bslash;n&quot;
comma
"&t;&t;&t;"
id|cfg-&gt;line_out_type
comma
...
c_func
(paren
l_string|&quot;==&gt; Best config: lo_type=%d, wired=%d, mio=%d&bslash;n&quot;
comma
"&t;&t;"
id|cfg-&gt;line_out_type
comma
...
multiline_comment|/* replace the channels in the composed amp value with the given number */
r_int
r_int
id|amp_val_replace_channels
c_func
...
multiline_comment|/* return the channel bits suitable for the given path-&gt;ctls[] */
r_int
id|get_default_ch_nums
c_func
(paren
r_struct
id|hda_codec
...
l_int|1
suffix:semicolon
multiline_comment|/* mono (left only) */
"&t;"
r_if
c_cond
(paren
...
suffix:semicolon
"&t;"
multiline_comment|/* bind volume control will be created in the case of dac = 0 */
"&t;"
r_if
c_cond
(paren
...
suffix:semicolon
multiline_comment|/* with individual volumes */
"&t;&t;"
r_else
"&t;&t;&t;"
id|dac
...
suffix:semicolon
"&t;"
multiline_comment|/* Let&squot;s create a bind-controls for volumes */
"&t;"
id|ctl
op_assign
id|new_bind_ctl
...
multiline_comment|/* check whether a control with the given (nid, dir, idx) was assigned */
r_bool
id|is_ctl_associated
c_func
(paren
r_struct
id|hda_codec
...
multiline_comment|/* can have the amp-in capability? */
r_bool
id|has_amp_in
c_func
(paren
...
multiline_comment|/* only for input pins */
"&t;&t;"
r_return
l_bool|false
suffix:semicolon
...
multiline_comment|/* can have the amp-out capability? */
r_bool
id|has_amp_out
c_func
(paren
...
multiline_comment|/* only for output pins */
"&t;&t;"
r_return
l_bool|false
suffix:semicolon
...
multiline_comment|/* check whether the given (nid,dir,idx) is active */
r_bool
id|is_active_nid
c_func
(paren
...
multiline_comment|/* get the default amp value for the target state */
r_int
id|get_amp_val_to_activate
c_func
(paren
r_struct
id|hda_codec
...
multiline_comment|/* initialize the amp value (only at the first time) */
r_void
id|init_amp
c_func
(paren
...
suffix:semicolon
"&t;"
id|snd_hda_codec_amp_init_stereo
c_func
(paren
id|codec
comma
...
suffix:semicolon
"&t;"
multiline_comment|/* here is a little bit tricky in comparison with activate_amp_out();&n;&t; * when aa-mixer is available, we need to enable the path as well&n;&t; */
"&t;"
r_for
c_loop
(paren
...
multiline_comment|/* configure the path from the given dac to the pin as the proper output */
r_void
id|alc_auto_set_output_and_unmute
c_func
(paren
...
multiline_comment|/* check whether the given pin can be a multi-io pin */
r_bool
id|can_be_multiio_pin
c_func
(paren
r_struct
id|hda_codec
...
multiline_comment|/*&n; * multi-io helper&n; *&n; * When hardwired is set, try to fill ony hardwired pins, and returns&n; * zero if any pins are filled, non-zero if nothing found.&n; * When hardwired is off, try to fill possible input pins, and returns&n; * the badness value.&n; */
r_int
id|alc_auto_fill_multi_ios
c_func
(paren
...
id|badness
suffix:semicolon
multiline_comment|/* no badness if nothing found */
"&t;"
)brace
"&t;"
r_if
...
(brace
"&t;&t;"
multiline_comment|/* cancel newly assigned paths */
"&t;&t;"
id|spec-&gt;paths.used
op_sub_assign
id|spec-&gt;multi_ios
...
"&t;"
multiline_comment|/* assign volume and mute controls */
"&t;"
r_for
c_loop
(paren
...
suffix:semicolon
"&t;"
multiline_comment|/* check the multiple speaker pins */
"&t;"
r_if
c_cond
(paren
...
multiline_comment|/* update mute-LED according to the speaker mute state via mic1 VREF pin */
r_void
id|alc269_fixup_mic1_mute_hook
c_func
(paren
r_void
op_star
...
multiline_comment|/* update mute-LED according to the speaker mute state via mic2 VREF pin */
r_void
id|alc269_fixup_mic2_mute_hook
c_func
(paren
r_void
op_star
...
comma
"&t;"
id|ALC269_FIXUP_MIC1_MUTE_LED
comma
"&t;"
id|ALC269_FIXUP_MIC2_MUTE_LED
comma
"&t;"
